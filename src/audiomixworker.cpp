...
QString AudioMixWorker::buildFilterStr() const
{
    ...
    // 源音频：在各区间时间窗内应用 srcVol
    s += "[in0]";
    bool anyReduce = false;
    for (const auto& r : task_.regions) {
        if (r.srcVol < 0.999f) { anyReduce = true; break; }
    }
    if (anyReduce) {
        for (int i = 0; i < N; ++i) {
            const auto& r = task_.regions[i];
            double startSec = r.videoStartUs / 1e6;
            double endSec   = (r.videoStartUs + r.audioDurationUs) / 1e6;
