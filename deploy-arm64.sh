...
echo "[deploy] 生成启动脚本 run.sh（设置 LD_LIBRARY_PATH 指向板卡 ffmpeg_rkmpp）..."
ssh "$BOARD_HOST" "cat > ${BOARD_DIR}/run.sh" <<'EOF'
#!/usr/bin/env bash
# 编译时 RPATH 指向本机 sysroot 路径，板卡上需手动指定 ffmpeg_rkmpp 库目录
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export LD_LIBRARY_PATH="$HOME/ffmpeg_rkmpp/lib:${LD_LIBRARY_PATH:-}"
